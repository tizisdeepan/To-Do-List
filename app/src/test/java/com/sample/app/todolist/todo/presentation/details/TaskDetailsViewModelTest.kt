...
class TaskDetailsViewModelTest {
    ...
    private val fetchTaskByIdUseCase = FetchTaskByIdUseCase(fakeTaskRepository)
    private val updateTaskUseCase = UpdateTaskUseCase(fakeTaskRepository)
    private val deleteTaskByIdUseCase = DeleteTaskByIdUseCase(fakeTaskRepository)
    ...
    fun setUp() {
        viewModel = TaskDetailsViewModel(SavedStateHandle().apply {
            set("ID", 1)
        }, rule.getDispatcherProvider(), fetchTaskByIdUseCase, updateTaskUseCase, deleteTaskByIdUseCase)
    }
